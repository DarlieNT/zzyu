...
export default async function handler(
  ...
  const { userId, reason, username } = req.body;
    ...
    if (!userId || !reason) {
      res.status(400).json({ error: '用户ID和封禁原因不能为空' });
      return;
    }
    ...
    const existingBan = bannedData.users.find(user => user.userId === userId);
    if (existingBan) {
      res.status(400).json({ error: '用户已被封禁' });
      return;
    }
    ...
    const newBannedUser: BannedUser = {
      ...
      bannedBy: userData.username || userData.sub
    };

    bannedData.users.push(newBannedUser);
    saveBannedUsers(bannedData);

    res.status(200).json({ success: true, message: '用户已封禁' });
  } catch (error) {
